...
AFD afd_finit(char *file){
  ...
  if(fichier!=NULL){
    ...
    while(fgets(chaine,10,fichier)){
      ...
      afd_ajouter_transition(A,Ei,Sk,Ej);
    }
    fclose(fichier);
    return A;
  }
...
int afd_simuler(AFD A, char *s){
  int i=A->delta[A->q0][A->dico[s[0]-ASCII_FIRST]];
  for(int k=1;k<strlen(s);k++){
    i=A->delta[i][A->dico[s[k]-ASCII_FIRST]];
  }
  for(int j=0;j<A->lenF;j++){
    if(i==A->F[j]){
      return 1;
    }
  }
